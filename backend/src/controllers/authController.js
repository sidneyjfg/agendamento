...
class AuthController {
  ...
  async register(req, res) {
    const { name, email, password, phone, role, slugPublicId, planId } = req.body; // Adicionando role no registro
    // Validação dos campos obrigatórios
    if (!name || !email || !password || !phone) {
      return res.status(400).json({ message: 'Nome, email, senha e telefone são obrigatórios.' });
    } try {
      const newUser = await authService.register(name, email, password, phone, role, slugPublicId, planId);
